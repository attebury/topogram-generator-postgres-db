...
assertNoEnvFilesInTarball(generatorTarball, "@topogram/generator-postgres-db");
...
writeJson(path.join(projectRoot, "package.json"), { name: "topogram-generator-postgres-db-consumer", private: true, type: "module", devDependencies: { "@topogram/cli": cliDependencySpec, "@topogram/generator-postgres-db": `file:${generatorTarball}` } });
...
const outputRoot = path.join(projectRoot, "app", "apps", "db", "app_postgres");
...
assert.equal(fs.existsSync(path.join(outputRoot, "drizzle", "schema.ts")), true, `Expected generated Drizzle schema`);
...
assert.match(fs.readFileSync(path.join(outputRoot, "prisma", "schema.prisma"), "utf8"), /provider = "postgresql"/);
assert.match(fs.readFileSync(path.join(outputRoot, "drizzle", "schema.ts"), "utf8"), /pgTable\("greetings"/);
run("bash", [path.join(outputRoot, "scripts", "db-status.sh")], { cwd: projectRoot, quiet: true });
console.log("Package-backed @topogram/generator-postgres-db smoke passed.");
